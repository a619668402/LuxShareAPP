...
public abstract class PermissionFragment extends Fragment {
    ...
    protected boolean isPermissionGranted(String permissionName, int requestCode) {
        ...
        // 如果没有授权就请求授权
        if (ContextCompat.checkSelfPermission(getContext(), permissionName) != PackageManager.PERMISSION_GRANTED) {
    ...
    protected boolean isPermissionAllGranted(String[] permissions, int requestCode) {
        ...
        if (denyCount != 0) {
            ActivityCompat.requestPermissions(getActivity(), denyList.toArray(new String[denyCount]), PermissionConstant.PERMISSION_ALL);
            return false;
        }
        return true;
    }
    ...
    private void showAlertDialog(String msgContent) {
        ...
        builder.setTitle("提示")
                ...
                .setPositiveButton("设置", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        Uri packUri = Uri.parse("package:" + getContext().getPackageName());
